"""Kaggle V3: add semantic XGBoost diversity to the converged V2 ensemble.
...
def detect_xgb_device() -> str:
    """Use a Kaggle GPU when one is attached, otherwise use CPU hist."""
    try:
...
def main() -> None:
    ...
    blend_candidates = []
    weight_steps = 20
    for native_units in range(weight_steps + 1):
        for semantic_units in range(weight_steps + 1 - native_units):
            xgb_units = weight_steps - native_units - semantic_units
            native_weight = native_units / weight_steps
            semantic_weight = semantic_units / weight_steps
            xgb_weight = xgb_units / weight_steps
            prediction = (
                native_weight * native_oof
                + semantic_weight * semantic_oof
                + xgb_weight * xgb_oof
            )
    ...
    xgb_weight = float(winner["xgb_weight"])
    blend_oof = (
        native_weight * native_oof
        + semantic_weight * semantic_oof
        + xgb_weight * xgb_oof
    ...
    oof_frame.to_csv(OUTPUT_DIR / "v3_oof.csv", index=False)
    test_prediction_frame.to_csv(OUTPUT_DIR / "v3_test_predictions.csv", index=False)
    pd.DataFrame(fold_rows).to_csv(OUTPUT_DIR / "v3_fold_metrics.csv", index=False)
    blend_frame.to_csv(OUTPUT_DIR / "v3_blend_search.csv", index=False)
    importance = pd.DataFrame(importance_rows)
    importance.groupby(["model", "feature"], as_index=False)["importance"].agg(
        mean="mean", std="std"
    ).sort_values(["model", "mean"], ascending=[True, False]).to_csv(
        OUTPUT_DIR / "v3_feature_importance.csv", index=False
    )
    ...
    pd.DataFrame(segments).to_csv(OUTPUT_DIR / "v3_segment_metrics.csv", index=False)
    ...
    with (OUTPUT_DIR / "v3_metrics.json").open("w", encoding="utf-8") as file:
        json.dump(metrics, file, indent=2)
    print("\nV3 complete", flush=True)
