"""Kaggle V1: native + fold-safe semantic LightGBM and OOF blending.
...
def fit_classifier(model, fit_x, fit_y, valid_x, valid_y):
    model.fit(
        fit_x,
        fit_y,
...
def main() -> None:
    ...
    for fold, (fit_index, valid_index) in enumerate(splits, start=1):
        ...
        native_model = make_classifier(SEED + fold)
        fit_classifier(
            native_model,
            native_fit,
        ...
        semantic_model = make_classifier(SEED + fold)
        fit_classifier(
            semantic_model,
            semantic_fit,
            target.iloc[fit_index],
        ...
        fold_rows.extend(
            [
                {"fold": fold, "model": "native", "auc": native_auc},
                {"fold": fold, "model": "semantic", "auc": semantic_auc},
            ]
        )
    ...
    oof_frame.to_csv(OUTPUT_DIR / "v1_oof.csv", index=False)
    test_prediction_frame.to_csv(OUTPUT_DIR / "v1_test_predictions.csv", index=False)
    pd.DataFrame(fold_rows).to_csv(OUTPUT_DIR / "v1_fold_metrics.csv", index=False)
    blend_frame.to_csv(OUTPUT_DIR / "v1_blend_search.csv", index=False)
    importance = pd.DataFrame(importance_rows)
    importance.groupby(["model", "feature"], as_index=False)["importance"].agg(
        mean="mean", std="std"
    ).sort_values(["model", "mean"], ascending=[True, False]).to_csv(
        OUTPUT_DIR / "v1_feature_importance.csv", index=False
    )
    ...
    pd.DataFrame(segments).to_csv(OUTPUT_DIR / "v1_segment_metrics.csv", index=False)
    ...
    with (OUTPUT_DIR / "v1_metrics.json").open("w", encoding="utf-8") as file:
        json.dump(metrics, file, indent=2)
    print("\nV1 complete", flush=True)
